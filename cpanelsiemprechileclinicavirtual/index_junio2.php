...
$twig = new Twig_Environment($loader, array('charset' => 'iso-8859-1'));
	...
	if ($_GET['slug']=="vidasecurity"){
		header("Location: https://www.clinicavirtual.com/vidasecurity");
		exit();
	}
	if ($_GET['slug']=="pesco"){
		header("Location: https://www.clinicavirtual.com/pesco");
		exit();
	}
	if ($_GET['slug']=="cruzverde"){
		header("Location: https://www.clinicavirtual.com/cruzverde");
		exit();
	}
	if ($_GET['slug']=="farmaciasahumada"){
		header("Location: https://www.clinicavirtual.com/farmaciasahumada");
		exit();
	}
	if ($_GET['slug']=="divisionsalud"){
		header("Location: https://www.clinicavirtual.com/divisionsalud");
		exit();
	}
	if ($_GET['slug']=="clinicavirtual"){
		header("Location: https://www.clinicavirtual.com/clinicavirtual");
		exit();
	}
	if ($_GET['slug']=="lipigas"){
		header("Location: https://www.clinicavirtual.com/lipigas");
		exit();
	}
	if ($_GET['slug']=="thbseguros"){
		header("Location: https://www.clinicavirtual.com/thbseguros");
		exit();
	}
	if ($_GET['slug']=="vidacel"){
		header("Location: https://www.clinicavirtual.com/vidacel");
		exit();
	}
	if ($_GET['slug']=="colaboradoresvidacel"){
		header("Location: https://www.clinicavirtual.com/colaboradoresvidacel");
		exit();
	}
				...
				if ($_POST['submitContacto']) {
					...
					if (!$frmError) {
						...
						if (!checkMail($_POST['contactoEmail'])) {
							...
							<b>Empresa:</b> {$fields['frmEmpresa']} <br />
							<b>Pais:</b> {$fields['frmPais']} <br />
							<b>Mensaje:</b> {$fields['frmMensaje']} <br />
							<b>Usuario:</b> {$fields['frmUsuario']} <br />
EOD;
